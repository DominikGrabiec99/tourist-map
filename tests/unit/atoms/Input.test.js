...
describe('Input.vue', () => {
  ...
  it('check label render', () => {
    expect(wrapper.get('[data-test="input-label"]').text()).toContain('Test Label');
  });

  it('check placeholder render', () => {
    expect(wrapper.get('[data-test="input"]').attributes('placeholder')).toContain('Test placeholder');
  });

  it('check error render', () => {
    expect(wrapper.get('[data-test="input-error"]').text()).toContain('Test error');
    expect(wrapper.get('[data-test="input"]').classes()).toContain('border-red-500');
  });

  it('check required star', () => {
    expect(wrapper.get('[data-test="input-label"]').classes()).toContain("after:content-['*']");
  });
